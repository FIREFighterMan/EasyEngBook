...
class excel_dearler:
	def __init__(self, path,data,txt_dealer,config):
		self.config = config
	...
	def select(self):
		...
		word_work = xlrd.open_workbook(r'allwords.xls')
		dst_work = copy(temp)
...
# .			yisi = data = sheetr.cell(row,4).value
			...
			if word =='':
				continue
			meanning = self.insert_mean_from_local(word)
			#meanning = get_wordmean(word)
			if len(meanning) == 0:
				meanning = 'NULL'
